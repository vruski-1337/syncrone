...
$stmt = $conn->prepare('SELECT * FROM doctors WHERE id = ? AND company_id = ?');
$stmt->bind_param('ii', $id, $cid);
$stmt->execute();
$doctor = $stmt->get_result()->fetch_assoc();
$stmt->close();

if (!$doctor) { setFlash('danger', 'Doctor not found.'); header('Location: doctors.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            $stmt = $conn->prepare('UPDATE doctors SET name=?, phone=?, specialization=?, commission_rate=?, notes=?, is_active=?, updated_at=NOW() WHERE id=? AND company_id=?');
            $stmt->bind_param('sssdsiii', $name, $phone, $special, $commission, $notes, $isActive, $id, $cid);
            $stmt->execute();
            $stmt->close();
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0 fw-bold"><i class="fas fa-user-md me-2"></i>Edit Doctor</h5>
        ...
        <form method="POST" novalidate>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="specialization" class="form-control" value="<?= sanitize($d['specialization'] ?? '') ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold">Commission Rate (%)</label>
                    <input type="number" name="commission_rate" class="form-control" min="0" max="100" step="0.01" value="<?= sanitize($d['commission_rate'] ?? '0') ?>">
                </div>
                <div class="col-12">
                    <label class="form-label fw-semibold">Notes</label>
                    <textarea name="notes" class="form-control" rows="3"><?= sanitize($d['notes'] ?? '') ?></textarea>
                </div>
                <div class="col-12">
